...
factor_labeling = read.table("salmon.gene.counts.matrix.bleached-44_vs_non-bleached-44.edgeR.DE_results.P0.05_C1.DE.subset", row.names=1)
...
# get gene lengths
gene_lengths = read.table("/gscratch/scrubbed/samwhite/outputs/20211015_mcap_DEG_pairwise_comparisons/bleached-44_non-bleached-44/Trinity.gene_lengths.txt", header=T, row.names=1, com='')
gene_lengths = as.matrix(gene_lengths[,1,drop=F])


# get background gene list
background = read.table("salmon.gene.counts.matrix.bleached-44_vs_non-bleached-44.edgeR.count_matrix", header=T, row.names=1)
...
go_enrich_filename = paste("salmon.gene.counts.matrix.bleached-44_vs_non-bleached-44.edgeR.DE_results.P0.05_C1.DE.subset", '.GOseq.enriched', sep='')
    ...
    go_depleted_filename = paste("salmon.gene.counts.matrix.bleached-44_vs_non-bleached-44.edgeR.DE_results.P0.05_C1.DE.subset", '.GOseq.depleted', sep='')
