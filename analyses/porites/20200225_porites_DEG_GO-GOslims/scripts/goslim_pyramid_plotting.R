...
df1 <- read.csv("analyses/20190603_all_salmon_DEG_b_vs_nb_GO/P0.05_C1.bleached-UP.subset.GOseq.enriched.flattened.FDR_1.0.BP.GOslims.csv")

# Read in second comparison file
df2 <- read.csv("analyses/20190603_all_salmon_DEG_b_vs_nb_GO/P0.05_C1.non-bleached-UP.subset.GOseq.enriched.flattened.FDR_1.0.BP.GOslims.csv")
...
df1$treatment <- 'non-bleached'
df2$treatment <- 'bleached'
...
ggplot(df3, aes(x = Term, fill = treatment, 
  ...
  labs(title = "Percentages of GO terms in GOslims", x = "GOslim", y = "Percent GO terms") + 
