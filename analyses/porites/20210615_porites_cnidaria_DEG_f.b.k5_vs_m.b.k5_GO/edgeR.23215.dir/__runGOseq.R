...
factor_labeling = read.table("salmon.gene.counts.matrix.female_vs_male.edgeR.DE_results.P0.05_C1.DE.subset", row.names=1)
...
# get gene lengths
gene_lengths = read.table("/media/sam/4TB_toshiba/porites/20210615_porites_cnidaria_DEG_f.b.k5_vs_m.b.k5_GO/Trinity.gene_lengths.txt", header=T, row.names=1, com='')
gene_lengths = as.matrix(gene_lengths[,1,drop=F])


# get background gene list
background = read.table("salmon.gene.counts.matrix.female_vs_male.edgeR.count_matrix", header=T, row.names=1)
...
go_enrich_filename = paste("salmon.gene.counts.matrix.female_vs_male.edgeR.DE_results.P0.05_C1.DE.subset", '.GOseq.enriched', sep='')
    ...
    go_depleted_filename = paste("salmon.gene.counts.matrix.female_vs_male.edgeR.DE_results.P0.05_C1.DE.subset", '.GOseq.depleted', sep='')
