...
factor_labeling = read.table("salmon.gene.counts.matrix.bleached-female_vs_non-bleached-female.edgeR.DE_results.P0.05_C1.DE.subset", row.names=1)
...
# get gene lengths
gene_lengths = read.table("/media/sam/4TB_toshiba/porites/20191219_all_DEG_f.b_vs_f.nb_44_GO/Trinity.gene_lengths.txt", header=T, row.names=1, com='')
gene_lengths = as.matrix(gene_lengths[,1,drop=F])


# get background gene list
background = read.table("salmon.gene.counts.matrix.bleached-female_vs_non-bleached-female.edgeR.count_matrix", header=T, row.names=1)
...
GO_info = read.table("/media/sam/4TB_toshiba/porites/20190530_trinotate_porites_all/go_annotations.txt", header=F, row.names=1,stringsAsFactors=F)
...
go_enrich_filename = paste("salmon.gene.counts.matrix.bleached-female_vs_non-bleached-female.edgeR.DE_results.P0.05_C1.DE.subset", '.GOseq.enriched', sep='')
    ...
    go_depleted_filename = paste("salmon.gene.counts.matrix.bleached-female_vs_non-bleached-female.edgeR.DE_results.P0.05_C1.DE.subset", '.GOseq.depleted', sep='')
